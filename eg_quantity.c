...
static void System_Color(ecs_iter_t *it)
{
	EgColor *c = ecs_term(it, EgColor, 1); // [out]
	for (int i = 0; i < it->count; i ++)
	{
		c[i].r = rand_range(0, 255);
		c[i].g = rand_range(0, 255);
		c[i].b = rand_range(0, 255);
		c[i].a = 255;
	}
}
...
void Module_EgQuantityImport(ecs_world_t *world)
{
	...
	ecs_set_component_actions(world, EgText, {
	...
	ecs_doc_set_brief(world, ecs_id(EgColor),
	"RGBA Color");
	ecs_struct_init(world, &(ecs_struct_desc_t) {
	.entity.entity = ecs_id(EgColor),
	.members = {
	...
	[in]    EgVelocity2F32,
	[out]   EgDrag2F32);
	ECS_TRIGGER(world, System_Color, EcsOnAdd,
	[out]   EgColor);
